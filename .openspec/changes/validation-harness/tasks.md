...
- [x] 1.1 Create `docs/validation/` directory
- [x] 1.2 Curate and commit `docs/validation/corpus.json` with exactly 50 PolitiFact claims distributed across all five ADR-0008 categories (10 per category; `CLAIMREVIEW_INDEXED` and `NOT_CLAIMREVIEW_INDEXED` mutually exclusive)
- [x] 1.3 Verify each entry has required fields: `id`, `claim_text`, `ground_truth`, `categories`, `politifact_url`, `captured_date`, `speaker`, `domain`
...
- [x] 1.5 Write `docs/validation/corpus-schema.json` (JSON Schema for corpus fixture validation at harness load time)

## 2. Harness Package Setup

- [x] 2.1 Create `tests/validation/` directory with `__init__.py`
- [x] 2.2 Create `tests/validation/conftest.py` with pytest fixtures: corpus loader, API client (wrapping `httpx`), Redis client for stream inspection
- [x] 2.3 Add `pytest-asyncio`, `httpx`, and `jsonschema` to `pyproject.toml` dev dependencies

## 3. Accuracy Scorer

- [x] 3.1 Implement `tests/validation/scorer.py`:
...
- [x] 3.2 Implement `tests/validation/reporter.py`:
...
- [x] 3.3 Implement directional constraints:
...
- [x] 4.1 Implement `tests/validation/runner.py`:
...
- [x] 4.2 Implement sequential claim submission with parallel poll loop:
...
- [x] 5.1 Implement `tests/validation/baseline.py`:
...
- [x] 5.3 Implement `tests/validation/comparison.py`:
...
- [x] 6.1 Create `tests/validation/steps/` directory with `__init__.py`
- [x] 6.2 Implement step definitions for all 9 scenarios in `validation-baseline.feature` (harness exercises all 11 agents):
...
- [x] 7.1 Add `validate` target to `Makefile`:
  ```makefile
  validate:
...
- [x] 8.1 Implement human-readable stdout summary table (printed by scorer after every corpus run):
  ```
...
- [x] 9.1 Write unit tests for `scorer.py`: within-one-tier edge cases (exact match, adjacent tier, two-tier miss, UNVERIFIABLE), six-tier ordering
- [x] 9.2 Write unit tests for `comparison.py`: gap computation, NFR-020 thresholds, corpus drift detection
- [x] 9.3 Write unit tests for corpus schema validation: valid corpus, missing fields, wrong category distribution
