...
- [x] Temporal server is running (docker-compose includes `temporal` service)
- [x] PostgreSQL is running with `runs` table migrated (TypeORM entity from NestJS backend)
...
- [x] 2.1 Define `Phase` dataclass in `workflows/dag.py` with fields: `id`, `name`, `agents`, `mode`
- [x] 2.2 Define DAG constant: Phase 1 ingestion (3 agents, sequential), Phase 2 fanout (6 agents including source-validator, parallel), Phase 3 synthesis (2 agents, sequential) -- total 11 agents
- [x] 2.3 Write unit tests: agent count = 11, phase modes correct, no duplicate agents, source-validator in Phase 2

---

## 3. Workflow Input/Output Models

- [x] 3.1 Define `WorkflowInput` dataclass: `run_id`, `claim_id`, `session_id`, `claim_text`
- [x] 3.2 Define `WorkflowResult` dataclass: `run_id`, `final_status`, `verdict_id`, `agent_results`
- [x] 3.3 Define `AgentActivityInput` and `AgentActivityResult` dataclasses with agent name, terminal status, observation count, duration

---

## 4. Run Lifecycle (PostgreSQL Persistence)

- [x] 4.1 Define `RunStatusEnum` with values: `pending`, `ingesting`, `analyzing`, `synthesizing`, `completed`, `cancelled`, `failed`
- [x] 4.2 Implement valid transition map and `InvalidRunTransition` exception
- [x] 4.3 Implement `update_run_status` Temporal activity: validate transition, write to PostgreSQL via async SQLAlchemy
- [x] 4.4 Implement `cancel_run` activity: transition to `cancelled` with reason; no-op if already terminal
- [x] 4.5 Implement `fail_run` activity: transition to `failed` with error message; no-op if already terminal
- [x] 4.6 Implement `get_run_status` activity: read current status from PostgreSQL
- [x] 4.7 Write unit tests: valid transitions, invalid transitions raise exception, cancel/fail no-op on terminal states

---

## 5. Completion Register

- [x] 5.1 Implement `CompletionRegister` class with `_register: dict[str, str | None]` in `completion/register.py`
- [x] 5.2 Implement `register_agent()`, `mark_complete()` (idempotent), `is_phase_complete()`, `is_agent_complete()`, `get_status()`, `get_incomplete_agents()`
- [x] 5.3 Implement `reset()` to clear all state
- [x] 5.4 Implement `rebuild_completion_register` Temporal activity: XRANGE each agent stream for STOP messages, return dict of agent->status
- [x] 5.5 Implement `merge_from_rebuild()` to integrate rebuild results
- [x] 5.6 Write unit tests: phase complete logic, idempotent mark_complete, reset, rebuild with mocked Redis

---

## 6. Agent Activity (run_agent_activity)

- [x] 6.1 Implement `run_agent_activity` in `activities/run_agent.py`: look up agent handler by name, invoke `handler.run()`, return `AgentActivityResult`
- [x] 6.2 Implement stream lifecycle: verify START published before agent logic, verify STOP after
...
- [x] 7.1 Define default retry policy: `initial_interval=1s`, `backoff_coefficient=2.0`, `maximum_interval=30s`, `maximum_attempts=3`
- [x] 7.2 Define non-retryable error types: `InvalidClaimError`, `MissingApiKeyError`, `StreamNotFoundError`
- [x] 7.3 Configure activity timeouts: `start_to_close_timeout=120s`, `heartbeat_timeout=60s`, `schedule_to_close_timeout=300s`

---

## 8. ClaimVerificationWorkflow

- [x] 8.1 Implement workflow class with `@workflow.defn` decorator, `@workflow.run` method accepting `WorkflowInput`
- [x] 8.2 Implement Phase 1 sequential dispatch: `await workflow.execute_activity()` for ingestion-agent, claim-detector, entity-extractor
- [x] 8.3 Implement check-worthiness gate: if claim-detector returns `terminal_status="X"`, call `cancel_run` and return early
- [x] 8.4 Implement Phase 1->2 transition: call `update_run_status` to transition `ingesting -> analyzing`
- [x] 8.5 Implement Phase 2 parallel dispatch: `asyncio.gather()` over six `workflow.execute_activity()` calls
- [x] 8.6 Implement Phase 2->3 transition: call `update_run_status` to transition `analyzing -> synthesizing`
- [x] 8.7 Implement Phase 3 sequential dispatch for blindspot-detector, synthesizer
...
- [x] 13.1 Write stub agent handler fixture: publishes START/OBS/STOP, returns AgentActivityResult
- [x] 13.2 Full run with 11 stub agents: verify `completed` status, all streams contain START+STOP
- [x] 13.3 Check-worthiness gate: claim-detector returns X, verify `cancelled`, no Phase 2 agents dispatched
