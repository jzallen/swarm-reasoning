...
async def _phase_a_extract(state: PipelineState, config: RunnableConfig) -> dict[str, Any]:
    """Pure: URL → intake agent state dict (article meta + claims). No observation writes.

    Returns the agent's final state dict directly. The caller inspects
    ``error`` and ``extracted_claims`` via ``.get(...)`` to decide
    control flow.
    """
