...
type SessionPhase = 'idle' | 'creating' | 'active' | 'verdict' | 'frozen' | 'expired' | 'error';
...
function sessionReducer(state: SessionState, action: SessionAction): SessionState {
  switch (action.type) {
    ...
    case 'SESSION_CREATED':
      return { ...state, phase: 'active', sessionId: action.sessionId, claim: action.claim };
    case 'SESSION_LOADED':
    ...
    case 'SESSION_FROZEN':
      return { ...state, phase: 'frozen', snapshotUrl: action.snapshotUrl ?? null };
    case 'ERROR':
...
export function useSession() {
  ...
  const handleSubmit = useCallback(async (claimText: string) => {
    ...
    try {
      ...
      dispatch({ type: 'SESSION_CREATED', sessionId: session.sessionId, claim: claimText });
    } catch (err) {
      dispatch({ type: 'ERROR', message: err instanceof Error ? err.message : 'Failed to submit claim' });
    }
  }, []);
  ...
  const handleVerdictReady = useCallback(async (sessionId: string) => {
    try {
      ...
      dispatch({ type: 'ERROR', message: err instanceof Error ? err.message : 'Failed to load verdict' });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    getSession(pathSessionId)
      ...
      .catch((err) => {
        dispatch({ type: 'ERROR', message: err instanceof Error ? err.message : 'Session not found' });
      });
  }, []);
