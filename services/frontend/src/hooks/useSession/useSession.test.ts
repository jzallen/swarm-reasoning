...
describe("useSession", () => {
  ...
  describe("dispatch actions", () => {
    ...
    it("handles SESSION_FROZEN action", () => {
      ...
      expect(result.current.state.snapshotUrl).toBe(
        "https://cdn.example.com/snap.html",
      );
    });
  ...
  describe("URL-based session loading", () => {
    ...
    it("reloads session on popstate (browser back/forward)", async () => {
      const session = {
        sessionId: "12345678-1234-1234-1234-123456789abc",
        status: "frozen" as const,
        claim: "Back-nav claim",
        createdAt: "2026-01-01",
        snapshotUrl: "https://cdn.example.com/snap.html",
      };
      mockGetSession.mockResolvedValue(session);
      ...
      // Simulate browser back/forward to a session URL
      window.history.pushState(
        null,
        "",
        "/12345678-1234-1234-1234-123456789abc",
      ...
      expect(result.current.state.claim).toBe("Back-nav claim");
    });
