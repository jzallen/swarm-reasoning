...
# required JSON fields, error handling, and Consumer API delivery.
...
Feature: Verdict Publication
  ...
  Scenario: Publication activates when synthesizer publishes STOP with F-status
    ...
    Then the orchestrator notifies the Consumer API of run completion
    And the Consumer API begins reading observations from Redis Streams
  ...
  Scenario: Publication does not activate on preliminary verdict
    ...
    Then the Consumer API does NOT attempt to read finalized observations

  # ---------------------------------------------------------------------------
  # Observation Resolution During Publication
  # ---------------------------------------------------------------------------

  Scenario: Consumer API resolves corrections before constructing verdict response
    ...
    When the Consumer API constructs the verdict response
    Then the JSON field "claimreview_verdict" equals "FALSE"
    And the JSON does not contain "HALF_TRUE" for claimreview_verdict

  Scenario: Consumer API excludes X-status observations from verdict response
  ...
  Scenario: Coverage field contains sub-objects for left, center, and right
    When the Consumer API constructs a verdict response for a completed run
  ...
  Scenario: Publication succeeds when all required fields are present
    ...
    When the Consumer API validates the constructed verdict JSON
    Then schema validation passes
    And the verdict is persisted
    And the run status transitions to PUBLISHED
  ...
  Scenario: Verdict is queryable by run_id after successful publication
    Given the Consumer API has successfully published the verdict for run "RUN-001"
    When an analyst issues GET "/verdicts?run_id=RUN-001"
    Then the response status is 200
    And the response body matches the verdict response

  Scenario: Verdict is queryable by claim_id after successful publication
  ...
  Scenario: Run transitions to PUBLISHED after verdict is persisted
    Given the Consumer API constructs and validates the verdict for run "RUN-001"
    When the verdict is persisted
    Then the run status transitions to "PUBLISHED"
